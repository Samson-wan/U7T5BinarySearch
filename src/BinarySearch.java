  ...
  public static int binarySearchPrintedCommentary(int[] elements, int target)
  {
    ...
    public static int binarySearch(int[] a, int key){
        ...
        while (left <= right) {
            ...
            int middle = (left + right) / 2;
            System.out.println("Checking value at index " + middle + " (value = " + a[middle] + ")");

            if (key < a[middle]){
                right = middle - 1;
                System.out.println("-- " + a[middle] + " is too HIGH! Setting right index to: " + right + " (value = " + a[right] + ")");
            }
            else if (key > a[middle]) {
                left = middle + 1;
                System.out.println("-- " + a[middle] + " is too LOW! Setting left index to: " + left + " (value = " + a[left] + ")");
            }
            else {
                System.out.println("The target " + key + " was FOUND at index " + middle + "!  Returning " + middle);
                System.out.println("A total of " + numChecks + " elements were checked for a runtime of " + numChecks + " loop iterations");
                return middle;
            }
        }
        System.out.println("The target " + key + " was NOT found; returning -1");
        System.out.println("A total of " + numChecks + " elements were checked for a runtime of " + numChecks + " loop iterations");
        return -1;
    }
